<?php
    ...
    class DB extends PDO {
        ...
        {
            // $db=debug_backtrace();error_log( $db[0]['file'].' : '.$db[0]['line']." beginTransaction");
        ...
        {
            // $db=debug_backtrace();error_log( $db[0]['file'].' : '.$db[0]['line']." rollBack");
            $this->transactionLevel--;
            if ($this->transactionLevel === 0) {
        ...
        public function execute ($sql, $bind = null, $die_on_error = true)
        {
            ...
            #@file_put_contents(__DIR__ . '/_execute.log', json_encode(['sql' => $sql, 'bind' => $bind]) . PHP_EOL , FILE_APPEND);
            ...
            $timer = round (10 * (microtime (true) - $timer)) / 10;
            if ($timer > 2) {
            ...
            if (DEBUG || isset($_GET['debug'])) {
                ...
                exit ('Database error, check error log');
            }
        ...
        {
            // error_log("likeQuote before: $str");
            $str = trim ($this->quote ($str, PDO::PARAM_STR), "'");
            // error_log("likeQuote after quote: $str");
            $str = str_replace ('%', '\%', $str);
            $str = str_replace ('_', '\_', $str);
            // error_log("likeQuote after str_replace: $str");
            if ($lr == 'LR') {
                return "%$str%";
            }
            if ($lr == 'L') {
    ...
    define ('MYSQL_ERROR_DUPLICATE_KEY', 23000);
